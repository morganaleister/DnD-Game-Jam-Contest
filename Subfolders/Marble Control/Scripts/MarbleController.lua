...
function HandleMovement(dt)
    ...
    if setVel and newVel.size ~= 0 and newAVel.size ~= 0 then
        ...
        end -- less push force if airborne

        local missingVelocity = newVel - Vector3.New(vel.x, vel.y, 0)

        ball:SetVelocity(vel + (missingVelocity * dt * currentMovementRampUp))
    ...
    if not setVel and newAVel.size == 0 then
...
function Spacebar()
    if isGrounded() and jumpLockout <= 0 then
        local vel = ball:GetVelocity()
        timeSinceGrounded = 1
        ball:SetVelocity(Vector3.New(vel.x, vel.y, jumpVelocity))
        local sfx = World.SpawnAsset(JUMP_SFX, {parent = ball})
        jumpLockout = 0.1
    end
end
